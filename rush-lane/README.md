...
`node test/hiscore.js` extracts the high-score module and the `m:ss.t` time formatter out of `index.html` and checks ordering (fastest first), the top-10 cap and initials normalisation. `node test/race.js` slices `src/util/MathUtils.ts` out of the bundle -- the one module with no Three.js, no DOM and no imports -- and checks the lap and placement primitives: that `progressDelta` reads a step from 0.99 to 0.01 as +0.02 rather than -0.98, that a full lap accumulates to exactly 1 from any starting point and step size, that reversing back over the start line returns the accumulator to where it was rather than awarding a lap, and that a lap crosses the finish line exactly once however finely it is stepped. There is still no harness for the driving model or the track geometry, which cannot be built without a renderer.
